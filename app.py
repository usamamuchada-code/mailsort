...
def process_in_background(bid: str, pdf: Path, note: str):
    ...
    try:
        ...
        for e in r["emails"]:
            ...
            emails.append({**e, "package": pkg, "siu_missing": missing,
...
<a href="/">Batches</a><a href="/history">Search &amp; History</a><a href="/clients">Client database</a><a href="/settings">Settings</a><a href="/logout" style="margin-left:auto">Sign out</a><span style="color:#c4c4c4;font-size:11px">v28c</span></header>
...
<td><a class="btn small secondary" href="/batch/{{bid}}/client_zip/{{loop.index0}}"{% if e.siu_missing %} onclick="return siuWarn('{{e.siu_missing|join(", ")}}')"{% endif %}>PDFs</a> <a class="btn small secondary" href="/batch/{{bid}}/email/{{loop.index0}}">Preview</a>
...
<td>{{L.client.company_name if L.client else L.recipient_company or "—"}}</td>
...
def history():
    ...
    for p in sorted(BATCHES.iterdir(), reverse=True):
        ...
        for L in letters:
            ...
            if q:
                hay = " ".join([name, L.get("recipient_company") or "", L.get("address") or "",
                                L.get("sender") or "", L.get("summary") or ""]).lower()
                if q in hay:
