...
<a href="/">Batches</a><a href="/history">Search &amp; History</a><a href="/clients">Client database</a><a href="/templates">Templates</a><a href="/audit">Audit</a><a href="/settings">Settings</a><a href="/logout" style="margin-left:auto">Sign out</a><span style="color:#c4c4c4;font-size:11px">v28u</span></header>
...
{% for L in b.letters %}<tr class="lrow {{'review' if L.needs_review else ('sent' if L.downloaded_at else '')}}" data-id="{{L.letter_id}}" data-st="{% if L.match_state == 'review' %}check {% endif %}{% if not L.client %}held {% endif %}{% if L.client and L.match_state == 'verified' %}verified {% endif %}{% if L.emailed_at %}sent {% endif %}{% if not L.siu_ok %}siu {% endif %}">
