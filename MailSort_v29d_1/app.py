...
import csv, io, json, os, re, smtplib, socket, threading, urllib.error, uuid, datetime as dt
...
def ukd(v) -> str:
    """Show any stored date/timestamp as DD/MM/YYYY (with HH:MM kept for timestamps).
    Display-only – stored values stay as they are."""
    if not v:
        return ""
    s = str(v).strip()
    d = ms.parse_date(s[:10])
    if not d:
        return s.replace("T", " ")
    rest = s[10:].replace("T", " ").strip()
    t = rest[:5] if len(rest) >= 5 and rest[2:3] == ":" else ""
    return d.strftime("%d/%m/%Y") + ((" " + t) if t else "")


app.add_template_filter(ukd, "ukdate")
...
def parse_client_upload(raw: bytes) -> list[dict]:
    ...
    alias = {"company": "company_name", "name": "company_name", "client": "company_name",
             ...
             "business_address": "address", "virtual_address": "address", "office_address": "address",
             "registered_office": "address", "registered_office_address": "address", "service_address": "address",
             "mailing_address": "address", "mail_address": "address", "postal_address": "address",
             "address_on_file": "address", "address_held": "address", "our_address": "address",
             "company_address": "address", "trading_address": "address", "correspondence_address": "address",
             "siu_office": "siu", "office": "siu", "unit_no": "siu", "room": "siu", "room_number": "siu"}
    out = []
    for r in rows:
        n = {}
        for k, v in r.items():
            ...
            val = (v or "").strip()
            if val or key not in n:   # two source columns mapping to one field: keep the filled one
                n[key] = val
        if not n.get("address"):
            # exports often split the address over several columns – stitch them together
            parts = [n.get(k) for k in ("address_line_1", "address_line1", "address1", "address_line_2",
                                        "address_line2", "address2", "address_line_3", "address_line3",
                                        "street", "city", "town", "county", "postcode", "post_code",
        ...
        out.append(n)
    # Detect whether this file writes slash dates month-first (US, e.g. 8/19/2026) or
    # day-first (UK). One style per file: if any date proves month-first (second number
    # over 12) and none proves day-first, the whole file is read as month-first.
    first_gt12 = second_gt12 = False
    for r in out:
        m = re.match(r"\s*(\d{1,2})[/\-.](\d{1,2})[/\-.]\d{2,4}", r.get("start_date") or "")
        if m:
            if int(m.group(1)) > 12: first_gt12 = True
            if int(m.group(2)) > 12: second_gt12 = True
    prefer = "mdy" if (second_gt12 and not first_gt12) else "dmy"
    for r in out:
        d = ms.parse_date(r.get("start_date", ""), prefer=prefer)
        if d:
            r["start_date"] = d.isoformat()   # store consistently as YYYY-MM-DD
...
def send_renewal_reminders(manual_reseller: str | None = None) -> tuple[int, list[str]]:
    ...
    for reseller, cs in by_res.items():
        ...
        lines = "\n".join(f"  - {c['company_name']} – renewal due {ukd(c['_expiry'])}" + _left(c) for c in cs)
...
def compliance_list() -> list[dict]:
    ...
    for c in read_clients():
        ...
        if issues:
            c["_issues"] = issues
            c["_last_notice"] = ukd((notices.get(c["company_name"]) or {}).get("sent_at", "")[:16])
            out.append(c)
    return out
...
def issues_email_parts(b: dict, e: dict, cfg: dict, selected=None) -> tuple[str, str]:
    """selected = set of issue indexes to include (None = all). The email renumbers itself."""
    issues = client_issues(b, e)
    if selected is not None:
        issues = [x for idx, x in enumerate(issues) if idx in selected]
...
<a href="/">Batches</a><a href="/history">Search &amp; History</a><a href="/clients">Client database</a><a href="/templates">Templates</a><a href="/audit">Audit</a><a href="/settings">Settings</a><a href="/logout" style="margin-left:auto">Sign out</a><span style="color:#c4c4c4;font-size:11px">v29d</span></header>
...
{% for b in batches %}<tr><td>{{b.id}}</td><td>{{b.created|ukdate}}</td><td>{{b.pdf}}<br><span class="muted">{{b.note}}</span></td>
...
{% else %}
<p>{{b.summary}} &middot; {{b.pages}} pages &middot; processed from <b>{{b.pdf}}</b> {{b.created|ukdate}}
...
<td>{{e.letters}}</td><td>{{e.action}}{% if e.siu_missing %}<br><span class="pill high">⚠ SIU missing on {{e.siu_missing|join(", ")}}</span>{% endif %}</td><td>{% if e.sent_at %}{{e.sent_at|ukdate}} <span class="muted">(emailed)</span>{% elif e.manual_sent_at %}{{e.manual_sent_at|ukdate}} <span class="muted">(marked by staff)</span>{% else %}–{% endif %}</td>
...
{% if e.issues_sent_at %}<span class="muted" style="font-size:11px">sent {{e.issues_sent_at|ukdate}}</span>{% endif %}{% endif %}</div></td></tr>{% endfor %}</table>
<p style="margin-top:14px" class="btnrow"><form method="post" action="/batch/{{bid}}/send_all" onsubmit="return confirm('Send to every ACTIVE client that has not been emailed yet?')">
<button class="btn">Send all active &amp; unsent</button></form>
{% set n_issue_clients = b.emails|selectattr("n_issues")|list|length %}
{% if n_issue_clients %}<form method="post" action="/batch/{{bid}}/issues_send_all" onsubmit="return confirm('Send an issues/reminder email to all {{n_issue_clients}} client(s) with outstanding issues (KYC not done, service expired, SIU missing, letters outside package)?') && confirm('Second check – this emails {{n_issue_clients}} customer(s) at once. Clients already notified for this batch are skipped. Continue?')">
<button class="btn" style="background:#b91c1c">⚠ Send issues emails to all affected ({{n_issue_clients}})</button></form>{% endif %}</p></div>
...
<table><tr><th>ID</th><th>Addressee (as printed)</th><th>Matched client</th><th>Address on letter (AI read)</th><th>Address on file (CSV)</th><th>Status</th><th>Sender</th><th>Summary</th><th>Actions</th><th></th></tr>
...
<td style="white-space:nowrap"><b>{{L.letter_id}}</b><br><span class="muted tiny">{{L.pages|length}} page{{'s' if L.pages|length != 1 else ''}} · p.{{L.pages|join("-")}}</span></td>
...
<br><span class="muted tiny">{% if L.client.email %}✉ will send to {{L.client.email}}{% else %}<span class="pill high" style="font-size:11px">no email on file</span>{% endif %}</span>
...
<td style="font-size:12px;min-width:150px">{% if L.address %}{{L.address}}{% else %}<span class="muted">—</span>{% endif %}</td>
<td style="font-size:12px;min-width:130px">{% if L.client and L.client.address %}{{L.client.address}}{% elif L.client %}<span class="pill high" style="font-size:11px">no address in CSV</span>{% else %}<span class="muted">—</span>{% endif %}</td>
...
</div>{% if L.emailed_at %}<span class="muted tiny">✉ {{L.emailed_at|ukdate}}</span>{% endif %}</td>
<td><button type="button" class="btn small secondary" onclick="tgl('det-{{L.letter_id}}')">Details</button></td></tr>
<tr class="drow" id="det-{{L.letter_id}}" hidden><td colspan="10"><div class="dgrid">
...
<div><b>Email it will be sent to</b>{% if L.client and L.client.email %}{{L.client.email}}{% if L.emailed_to %} <span class="muted tiny">(sent to {{L.emailed_to}})</span>{% endif %}{% elif L.client %}<span class="pill high" style="font-size:11px">no email on file</span>{% else %}<span class="muted">— no client matched —</span>{% endif %}</div>
...
<div><b>Tracking</b>{% if L.downloaded_at %}<span class="pill active">✔ downloaded {{L.downloaded_at|ukdate}}</span>{% elif L.opened_at %}<span class="pill">👁 opened ×{{L.opens or 1}} · {{L.opened_at|ukdate}}</span>{% else %}<span class="muted">not opened yet</span>{% endif %}</div>
...
<form method="post" action="/batch/{{bid}}/issues_send/{{i}}" onsubmit="return confirm('Send this issues email to {{e.email}}?')">
<input type="hidden" name="has_sel" value="1">
<p style="margin-bottom:4px"><b>Tick the issues to include in the email:</b></p>
{% for x in issues %}<label style="display:flex;gap:8px;align-items:flex-start;margin:6px 0;font-weight:400;font-size:14px">
<input type="checkbox" name="sel" value="{{loop.index0}}" checked style="width:auto;margin-top:3px;transform:scale(1.2)"> <span>{{x}}</span></label>{% endfor %}
<p class="muted tiny">Unticked issues are left out and the email renumbers itself automatically. The preview below shows the email with every issue included.</p>
<pre style="white-space:pre-wrap;background:#f9fafb;padding:14px;border-radius:8px">{{body}}</pre>
<p><a class="btn secondary" href="/batch/{{bid}}">Back</a>
<button class="btn" {% if not e.email %}disabled{% endif %}>Send issues email</button>
{% if e.issues_sent_at %}<span class="muted">Already sent {{e.issues_sent_at|ukdate}} – sending again is allowed.</span>{% endif %}</p></form></div>{% endblock %}"""
...
<p><b>The address you hold for each client goes in the <code>address</code> column</b> (e.g. <i>Unit A80, 4-6 Greatorex Street, London E1 5NF</i>), and their unit number in <code>siu</code> (e.g. <i>A80</i>) – these two columns are what lets MailSort prove a letter belongs to a client instead of guessing by name.
The upload understands many header spellings for it (address, business address, registered office, virtual address, …) and will stitch together <i>address line 1 / line 2 / city / postcode</i> columns automatically.</p>
...
<p class="muted">{{clients|length}} clients on file{% if mtime %} · last updated {{mtime|ukdate}}{% endif %} · <a href="/clients/download">download current CSV</a></p>
...
<div><label>Address we hold (unit + building)</label><input type="text" name="address" placeholder="Unit A80, 4-6 Greatorex Street, London E1 5NF"></div>
...
<td>{{c.company_name}}</td><td>{{c._expiry|ukdate}}</td><td>{% if c._days_left < 0 %}<span class="pill high">overdue</span>{% else %}{{c._days_left}}{% endif %}</td>
<td>{% if c._reminded %}<span class="pill active">✔ {{c._reminded|ukdate}}</span>{% else %}<span class="muted">not yet</span>{% endif %}</td>
...
<td>{% if c.start_date and not c._expiry %}<span class="pill high" title="This date could not be understood – use YYYY-MM-DD or DD/MM/YYYY">⚠ {{c.start_date}}</span>{% else %}{{c.start_date|ukdate or "—"}}{% endif %}</td>
<td>{% if c._expiry %}{{c._expiry|ukdate}}{% if c._auto_overdue %} <span class="pill high">passed</span>{% endif %}{% else %}<span class="muted">—</span>{% endif %}</td></tr>{% endfor %}</table></div>
<script>
function cfilt(){
...
<td>{{L.created|ukdate}}</td><td><a href="/batch/{{L.batch}}">{{L.batch}}</a></td>
...
{% for d in downloaded %}<tr><td>{{d.downloaded_at|ukdate}}</td>
...
{% for s in sent %}<tr><td>{{s.when|ukdate}}</td><td>{% if s.how=='emailed' %}<span class="pill active">emailed</span>{% else %}<span class="pill">marked by staff</span>{% endif %}</td><td>{{s.company}}</td><td>{{s.sent_to or "—"}}</td><td>{{s.letters}}</td>
...
def issues_send(bid, i):
    ...
    cfg = load_config()
    selected = None
    if request.form.get("has_sel"):   # the preview page's tick boxes decide what goes in
        selected = {int(x) for x in request.form.getlist("sel") if x.isdigit()}
        if not selected:
            flash("No issues ticked – nothing was sent. Tick at least one issue to include in the email.")
            return redirect(f"/batch/{bid}/issues/{i}")
    subject, body = issues_email_parts(b, e, cfg, selected=selected)
    try:
...
@app.post("/batch/<bid>/issues_send_all")
def issues_send_all(bid):
    """Send the issues email to every client in this batch that has outstanding issues
    (KYC not done, service expired, SIU missing, letters outside package, …).
    Clients already notified for this batch are skipped so nobody is emailed twice."""
    b = load_batch(bid) or abort(404)
    cfg = load_config()
    ok, skipped, failed = 0, 0, []
    for e in b["emails"]:
        if not client_issues(b, e):
            continue
        if e.get("issues_sent_at"):
            skipped += 1
            continue
        if not e.get("email"):
            failed.append(f"{e['company']}: no email on file")
            continue
        try:
            ...
            failed.append(f"{e['company']}: {ex}")
    save_batch(bid, b)
    msg = f"Issues emails sent to {ok} client(s)."
    if skipped: msg += f" {skipped} skipped – already notified for this batch (use their Report issues button to re-send)."
    if failed: msg += " Problems: " + "; ".join(failed) + "."
    flash(msg)
    return redirect(f"/batch/{bid}")
...
C001,Bluebird Consulting Ltd,Sarah Khan,sarah@bluebirdconsulting.co.uk,active,Premium,2026-03-01,,,yes,A80,"Unit A80, 4-6 Greatorex Street, London E1 5NF"
C002,Northgate Logistics Limited,Tom Reid,tom@northgatelogistics.com,active,Standard,2026-01-15,FormationsHub Ltd,accounts@formationshub.com,yes,A81,"Unit A81, 4-6 Greatorex Street, London E1 5NF"
C003,Pixel & Pine Studio Ltd,Amira Osei,hello@pixelandpine.co.uk,active,Basic,2025-06-10,FormationsHub Ltd,accounts@formationshub.com,no,A82,"Unit A82, 4-6 Greatorex Street, London E1 5NF"
C004,Harrow Property Ventures LLP,James Whitfield,james@harrowpv.com,cancelled,Standard,2024-11-20,,,yes,A83,"Unit A83, 4-6 Greatorex Street, London E1 5NF"
C005,Greenleaf Nutrition Ltd,Priya Nair,priya@greenleafnutrition.com,active,Premium,2026-05-05,BizStart Agency,hello@bizstart.agency,no,A84,"Unit A84, 4-6 Greatorex Street, London E1 5NF"
"""
